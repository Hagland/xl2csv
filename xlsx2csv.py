...
def load_files(pattern, combine):
    ...
    files = [
        (f, openpyxl.load_workbook(f, data_only=True).active)
        for f in files
...
def write_separate(files, sep, encoding):
    for f in files:
        name, ws = f
        with open(
            name.replace("xlsx", "csv"), "w", encoding=encoding
        ) as csv_file:
...
@click.option(
    ...
    "--pattern",
    default="*.xlsx",
    help="Pattern to match input files, default *.xlsx",
)
...
@click.option(
    ...
    "--encoding",
    default="cp1252",
    help="The input file's encoding, default cp1252",
)
@click.option(
    "-c",
    "--combined",
    default=False,
    is_flag=True,
    help="Combine all inputs into a single output file",
)
@click.option(
    "-o",
    "--output",
    default="combined.csv",
    help="Output file for combined files",
)
